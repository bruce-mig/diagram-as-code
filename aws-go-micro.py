...
from diagrams.aws.database import ElastiCache, RDS, DocumentDB
...
with Diagram("aws go microservices", show=False):
    dns = Route53("dns")
    ...
    with Cluster("VPC"):
        ...
        with Cluster("(Network ACL) \n Public Subnet"):
            ...
            with Cluster("Services"):
                svc_group = [EKS("go micro1")]

        with Cluster("(Network ACL) \n Private Subnet"):
            pvsub = PrivateSubnet("Private Subnet \n CIDR: 10.0.1.0/24 \n (IPv4)")
            rabbit = RabbitMQ("rabbitMQ")
            rabbit - RabbitMQ("replica") << Edge(label="collect") << metrics
